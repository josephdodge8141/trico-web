...
test('canonical feature files are the catalog source rather than copied text', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      'public.seed-if-empty-preserves-edits',
      'public.seed-if-empty-rejects-unsafe-current::wrong-entity-id',
      'public.seed-if-empty-rejects-unsafe-current::wrong-page',
      'public.seed-if-empty-rejects-unsafe-current::invalid-value',
      'public.seed-if-empty-rejects-unsafe-current::invalid-version',
      'public.seed-if-empty-rejects-unsafe-current::epoch-baseline-mismatch',
