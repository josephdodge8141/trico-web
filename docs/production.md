...
Before activating a backend image, the workflow waits for its ECR Basic Scan on the exact digest and blocks images with HIGH or CRITICAL findings. Trusted previews apply the same gate to both candidate image digests before launching a task. Basic Scan results are the last completed scan snapshot, so a promotion can use findings from the image's earlier on-push scan; this gate does not provide continuous scanning or update old findings when new CVEs are published.

Rollout prerequisite: before the first deployment or trusted-preview run using these scan gates, deploy the updated `TricoWebDeliveryFoundation` CDK stack in the delivery account/`us-east-2` and wait for it to complete. That stack grants the release and trusted-preview roles `ecr:DescribeImageScanFindings` on only the required repositories. Starting either workflow before this foundation update completes will fail closed with an authorization error.
